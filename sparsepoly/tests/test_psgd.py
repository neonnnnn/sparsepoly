...
def psgd_epoch_slow(P, w, X, y, loss, regularizer, lams, degree, alpha, beta, gamma,
    ...
    for ii in range(n_minibatches):
        # pick a minibatch
        minibatch_indices = np.atleast_1d(indices_samples[ii*batch_size:(ii+1)*batch_size])
        ...
        for j in range(n_features):
            ...
            grad_P[j] = np.dot(grad_kernel, dloss * X_batch[:, j]) # (n_components, n_samples)
        grad_P *= lams
        ...
        # update
        regularizer.prox(
            P, eta_P * gamma / (1.0 + eta_P * beta), degree,
        )
        if fit_linear:
...
def psgd_slow(X, y, loss, regularizer, lams=None, degree=2, n_components=5,
              alpha=1.0, beta=1.0, gamma=1.0, max_iter=10, tol=1e-5, eta0=1.0,
              fit_linear=True, verbose=False, random_state=None, mean=False,
              shuffle=False, batch_size='auto',learning_rate=1,
              power_t=1.0, n_iter_no_change=5):
...
@pytest.mark.parametrize(
    ...
    product([2, 3, 4], [1, 5, 8, n_samples, 'auto'], ['constant', 'optimal'],
            [True, False], loss_reg, regularizers)
)
def test_fm_same_as_slow_reg(degree, batch_size, learning_rate, fit_linear,
    ...
    reg = SparseFactorizationMachineRegressor(
        ...
        fit_linear=fit_linear, alpha=1e-3, beta=1e-3, gamma=0.0,
        regularizer=regularizer, learning_rate=learning_rate, eta0=0.01,
        warm_start=False, tol=1e-3, max_iter=10, random_state=0,
        shuffle=False, solver="psgd", batch_size=batch_size) 
    with warnings.catch_warnings():
        warnings.simplefilter('ignore')
        reg.fit(X, y)
        P_fit_slow, w_fit_slow = psgd_slow(
            X, y, loss=loss, regularizer=regularizer, lams=reg.lams_, 
            degree=degree, n_components=n_components, alpha=1e-3, beta=1e-3,
            gamma=0.0, learning_rate=learning_rate, eta0=0.01, shuffle=False,
            max_iter=10, tol=1e-3, random_state=0, fit_linear=fit_linear,
            batch_size=batch_size) 
...
@pytest.mark.parametrize(
    "degree, batch_size, learning_rate, fit_linear, loss, regularizer",
    product([2, 3, 4], [1, 5, 8, n_samples, 'auto'], ['constant', 'optimal'],
            [True, False], loss_clf, regularizers)
)
def test_fm_same_as_slow_clf(degree, batch_size, learning_rate, fit_linear,
    ...
    reg = SparseFactorizationMachineClassifier(
        ...
        regularizer=regularizer, learning_rate=learning_rate, eta0=0.01,
        warm_start=False, tol=1e-3, max_iter=10, random_state=0,
        shuffle=False, solver="psgd", batch_size=batch_size, verbose=0,
        loss=loss) 
    with warnings.catch_warnings():
        warnings.simplefilter('ignore')
        reg.fit(X, y)
