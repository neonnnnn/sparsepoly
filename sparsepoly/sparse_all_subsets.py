...
class _BaseSparseAllSubsets(BaseSparsePoly, metaclass=ABCMeta):
    ...
    def __init__(self, loss='squared', n_components=2, solver="pcd", beta=1,
                 ...
                 shuffle=False, verbose=False, callback=None, n_calls=10,
                 random_state=None):
        self.loss = loss
    ...
    def _fit_pbcd(self, X, y, y_pred, col_norm_sq, regularizer,
                  loss_obj, rng):
        n_samples = X.get_n_samples()
        n_features = X.get_n_features()
        converged = False
...
class SparseAllSubsetsRegressor(_BaseSparseAllSubsets, SparsePolyRegressorMixin):
    ...
    def __init__(self, n_components=2, solver='pcd', beta=1, gamma=1, eta0=0.1,
                 mean=False, tol=1e-6, regularizer='omegati', warm_start=False,
                 init_lambdas='ones', max_iter=100, shuffle=False, verbose=False,
                 callback=None, n_calls=10, random_state=None):

        super(SparseAllSubsetsRegressor, self).__init__(
            'squared', n_components, solver, beta, gamma, eta0, mean, tol,
            regularizer, warm_start, init_lambdas, max_iter, shuffle, verbose,
...
class SparseAllSubsetsClassifier(_BaseSparseAllSubsets, SparsePolyClassifierMixin):
    ...
    def __init__(self, loss='squared_hinge', n_components=2, solver='pcd',
                 beta=1, gamma=1, eta0=0.1, mean=False, regularizer='omegati',
                 tol=1e-6, warm_start=False, init_lambdas='ones', max_iter=100,
                 shuffle=False, verbose=False, callback=None, n_calls=10,
                 random_state=None):
        super(SparseAllSubsetsClassifier, self).__init__(
            loss, n_components, solver, beta, gamma, eta0, mean, tol,
            regularizer, warm_start, init_lambdas, max_iter, shuffle,
